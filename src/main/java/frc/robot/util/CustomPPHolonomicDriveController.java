...
public class CustomPPHolonomicDriveController implements PathFollowingController {
	...
	private static boolean completeOverrideRotation = false;
	 ...
	 */
	public CustomPPHolonomicDriveController(PIDConstants translationConstants, PIDConstants rotationConstants,
			double period) {
	...
	public ChassisSpeeds calculateRobotRelativeSpeeds(Pose2d currentPose, PathPlannerTrajectoryState targetState) {
		...
		if (!this.isEnabled) { return ChassisSpeeds.fromFieldRelativeSpeeds(xFF, yFF, 0, currentPose.getRotation()); }
		...
		if (rotationTargetOverride != null) { targetRotation = rotationTargetOverride.get().orElse(targetRotation); }
		...
		if (xFeedbackOverride != null) { xFeedback = xFeedbackOverride.getAsDouble(); }
		if (yFeedbackOverride != null) { yFeedback = yFeedbackOverride.getAsDouble(); }
		if (rotFeedbackOverride != null) { rotationFeedback = rotFeedbackOverride.getAsDouble(); }

		// Account for overrides
		double xout = xFF + xFeedback, yout = yFeedback + yFF,
				rout = completeOverrideRotation ? rotationFeedback : rotationFeedback + rotationFF;

		return ChassisSpeeds.fromFieldRelativeSpeeds(xout, yout, rout, currentPose.getRotation());
	}
	...
	public static void setRotationTargetOverride(Supplier<Optional<Rotation2d>> rotationTargetOverride) {
		CustomPPHolonomicDriveController.rotationTargetOverride = rotationTargetOverride;
	}
	...
	public static void overrideXFeedback(DoubleSupplier xFeedbackOverride) {
		CustomPPHolonomicDriveController.xFeedbackOverride = xFeedbackOverride;
	}
	...
	public static void clearXFeedbackOverride() {
		CustomPPHolonomicDriveController.xFeedbackOverride = null;
	}
	...
	public static void overrideYFeedback(DoubleSupplier yFeedbackOverride) {
		CustomPPHolonomicDriveController.yFeedbackOverride = yFeedbackOverride;
	}
	...
	public static void clearYFeedbackOverride() {
		CustomPPHolonomicDriveController.yFeedbackOverride = null;
	}
	 ...
	 *
	 * @param rotationFeedbackOverride Double supplier that returns the desired rotation feedback
	 *                                 in radians/sec
	 */
	public static void overrideRotationFeedback(DoubleSupplier rotationFeedbackOverride) {
		CustomPPHolonomicDriveController.rotFeedbackOverride = rotationFeedbackOverride;
	}
	...
	public static void clearRotationFeedbackOverride() {
		CustomPPHolonomicDriveController.rotFeedbackOverride = null;
	}
	...
	public static void clearFeedbackOverrides() {
		...
		setCompleteOverrideRotationOutput(false);
	}

	/**
	 * Sets whether to completely override path angular velocity outputs.
	 *
	 * @param override Whether to completely override path angular velocity outputs.
	 */
	public static void setCompleteOverrideRotationOutput(boolean override) { completeOverrideRotation = override; }
}
